  ...
  connection.get(query1, [user, email], (err, row) => {
    ...
    if (row['count'] == 0){
      connection.run(query2, [user, password, email, image_url], function(err) {
        ...
        res.status(200).send(this.lastID.toString()) //no entiendo lastID
      });
    }else{
      connection.close();
...
//MI IMPLEMENTACION PROPIA

app.get('/user/search_id', async (req, res, next) => {
  // data
  var user = req.query.user;
  var password = req.query.password;
  // logic
  var query = `SELECT id FROM users WHERE user=? AND password=?`;
  let connection = dbApp();
  connection.get(query, [user, password], (err, row) => {
    if (err) {
      console.error(err);
      res.status(500).send('Ups, ocurrió un error');
    } else if (row) {
...
//MI IMPLEMENTACION PROPIA
app.post('/user/pass_recovery', (req, res) => {
  // data
  let email = req.body.email;
  // logic
  let connection = dbApp();
  let sql = `SELECT id, name, user, email, image_url, password FROM users WHERE email=?`;
  connection.get(sql, [email], (err, row) => {
    if (err) {
      console.error(err);
      res.status(500).send('Ups, ocurrió un error');
    } else if (row) {
      ...
      res.status(404).send('No se encontró ningún usuario con el correo electrónico especificado');
    }
    connection.close();
  });
});
...
app.post('/user/update', async (req, res, next) => {
  ...
  connection.get(sql, params, (err, row) => {
    if (err) {
      ...
      connection.close();
      res.status(200).send('Se actualizo el usuario en el backend')
    }
  });
});
*/
...
app.post('/user/update', async (req, res, next) => {
  ...
  // Verificar si el nuevo usuario esta en uso
  let checkUsernameQuery = 'SELECT * FROM users WHERE user = ? AND id != ?';
  connection.get(checkUsernameQuery, [user, id], (err, row) => {
    if (err) {
      console.error(err);
      res.status(500).send('Ups, ocurrió un error al verificar el nombre de usuario');
      connection.close();
    } else {
      if (row) {
        res.status(400).send('El nuevo nombre de usuario ya está en uso');
        connection.close();
      } else {
        // Verificar si el nuevo email esta en uso
        let checkEmailQuery = 'SELECT * FROM users WHERE email = ? AND id != ?';
        connection.get(checkEmailQuery, [email, id], (err, row) => {
          if (err) {
            console.error(err);
            res.status(500).send('Ups, ocurrió un error al verificar el correo de usuario');
            connection.close();
          } else {
            if (row) {
              res.status(400).send('El nuevo correo de usuario ya está en uso');
              connection.close();
            } else {
              // Actualizar usuario en la base de datos
              let updateQuery = 'UPDATE users SET user = ?, name = ?, email = ? WHERE id = ?';
              let params = [user, name, email, id];
              connection.run(updateQuery, params, (err) => {
                if (err) {
                  console.error(err);
                  res.status(500).send('Ups, ocurrió un error al actualizar el usuario');
                } else {
                  res.status(200).send('Se actualizó el usuario en el backend');
                }
                connection.close();
              });
            }
...
//IMPLEMENTACION PROPIA
app.post('/user/password_search', async (req, res, next) => {
  // Datos recibidos
  var id = req.body.id;
  var password = req.body.password;
  ...
  connection.get(sql, params, (err, row) => {
    if (err) {
      ...
      connection.close();
      if (row) {
        res.status(200).send('Contraseña válida');
      } else {
        res.status(404).send('La contraseña antigua no es correcta');
      }
    }
  });
});
...
app.post('/user/password', async (req, res, next) => {
  ...
  connection.get(sql, params, (err, row) => {
    if (err) {
      ...
      connection.close();
      res.status(200).send('Contraseña guardada en el Backend')
    }
  });
});
